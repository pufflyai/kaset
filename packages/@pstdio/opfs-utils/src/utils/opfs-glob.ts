...
function globToRegExp(glob: string, opts: { dot: boolean; caseSensitive: boolean }): RegExp {
  // Convert to a regex that matches the FULL path (anchored ^$)
  // Supports: **, *, ?, [..] (with leading !), and escapes.
  let re = "^";
  let i = 0;
  ...
  while (i < glob.length) {
    ...
    if (c === "*") {
      if (glob[i + 1] === "*") {
        ...
        // If dot=false, try to avoid matching a leading dot for a segment wildcard.
        re += opts.dot ? "[^/]*" : "(?!\\.)[^/]*";
        i++;
      }
      continue;
...
function parseCharClass(glob: string, start: number): { src: string; newIndex: number } {
  ...
  while (i < glob.length && glob[i] !== "]") {
    const ch = glob[i];
    // Keep ranges like a-z as-is; just escape regex specials except '-'
    if (ch === "\\") {
...
function expandBraces(input: string): string[] {
  // Simple, single-nesting brace expansion: "*. {ts,tsx}" -> ["*.ts","*.tsx"]
  const idx = findTopLevelBraces(input);
  if (!idx) return [input];

  const { start, end } = idx;
  const before = input.slice(0, start);
  const inside = input.slice(start + 1, end);
  const after = input.slice(end + 1);

  const parts = splitBraceParts(inside);
  const tail = expandBraces(after);
  const out: string[] = [];
  for (const p of parts) {
    for (const t of tail) out.push(before + p + t);
  }
  return out;
}

function findTopLevelBraces(s: string): { start: number; end: number } | null {
  let depth = 0;
  let start = -1;
...
function splitBraceParts(s: string): string[] {
