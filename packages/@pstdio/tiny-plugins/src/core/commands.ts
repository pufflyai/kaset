...
export class CommandRegistry {
  ...
  register(pluginId: string, defs: CommandDefinition[] | undefined, handlers: Record<string, CommandHandler> | undefined) {
  ...
  list(pluginId: string): CommandDefinition[] {
    return [...(this.map.get(pluginId)?.values() ?? [])].map((entry) => entry.def);
  }
