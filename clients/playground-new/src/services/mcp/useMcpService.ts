...
export function useMcpClient(options: UseMcpClientOptions = {}): UseMcpClientState {
  const { serverUrl, accessToken, enabled = true } = options;
...
export function useMcpService(): UseMcpServiceResult {
  const servers = useWorkspaceStore((state) => state.settings.mcpServers);
  const activeIds = useWorkspaceStore((state) => state.settings.activeMcpServerIds);
