const u = "tiny-ui-", l = "tiny-ui-bundles-v1", c = "/tiny-ui/runtime.html", h = "/virtual/", r = self, d = (t) => {
  ...
  if (a) return a;
  const s = new URL(e.url), n = await t.match(s.pathname);
  if (n) return n;
  if (s.pathname === c) {
...
r.addEventListener("fetch", (t) => {
  const e = t, { request: a } = e;
  d(a) && e.respondWith(
    (async () => {
      const s = await caches.open(l), n = await f(s, a);
      if (n) return n;
